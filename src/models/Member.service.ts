...
class MemberService {
	...
	/** Bosh sahifadagi qisqa statistikalar uchun */
	public async getHomeStats(): Promise<{
		totalBooks: number
		totalMembers: number
		currentlyBorrowed: number
	}> {
		const [totalBooks, totalMembers, currentlyBorrowed] = await Promise.all([
			this.productModel.countDocuments({
				productStatus: ProductStatus.PROCESS,
			}),
			this.memberModel.countDocuments(),
			this.productModel.aggregate([
				{ $group: { _id: null, sum: { $sum: '$productRent' } } },
			]),
		])
		return {
			totalBooks,
			totalMembers,
			currentlyBorrowed: currentlyBorrowed[0]?.sum ?? 0,
		}
	}
}
