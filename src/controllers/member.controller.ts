...
class MemberController {
  ...
  public goHome = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const products = await memberService.getProducts({ page: 1, limit: 8 });
      res.render("home", { member: req.session.member ?? null, products });
    } catch (err) {
      next(err);
    }
  };
  ...
  public getProductsPage = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const page = Number(req.query.page) || 1;
      const search = (req.query.search as string) || "";
      const collection = (req.query.collection as ProductCollection) || undefined;
